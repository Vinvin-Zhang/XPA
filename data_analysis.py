...
def dcd_interface_dna_protein_full(read_path,out_path):
    ...
    for dcd_file in all_dcd:
        ...
        out_file = out_path + '/'+ 'full_interface_' + dcd_file.replace('dcd','txt')
...
def hist_residue_interface_dna(out_path,figure_path):
    print("\nhist_residue_interface_dna")
    total_len = 871
    DNA_len   = 598
    discard_line =  2000
    os.chdir(out_path)
    all_files = os.listdir('.')
    inpfile_str=["full_interface_xpa_dna","truncated_interface_xpa_dna"]
    inpfile_str=["full_interface_xpa_dna"]
    outfile_str={}
    ...
    for name in outfile_str.keys():
        i = i + 1
        residues_list = [0]*(total_len-DNA_len)
        line_num = 0
        for f in outfile_str[name]:
            ...
            for line in lig_lines:
                for each in line.split():
                    residues_list[int(each) -DNA_len -1] += 1

        residues_list = np.array(residues_list)
        p_list = residues_list*1.0/line_num
        fig = plt.figure(i)
        ax = fig.add_subplot(1,1,1)
        ax.plot(range(1,total_len-DNA_len+1),p_list)
        ax.set_title(name)
...
##<<<<<<<<<-------------------------
##<<<<<<<<<-------------------------
## the snapshots with the contact formed between DNA and protein is considered. 
def diffusion_1D(out_path,figure_path):
    ...
    os.chdir(out_path)
    all_files = os.listdir('.')
    inpfile_str=["truncated_interface_xpa_dna"]
    outfile_str={}
    for s in inpfile_str:
        for f in all_files:
            if not( s in f):
    ...
    for name in names:
        ...
        D_t.insert(0,0)
        all_msd.insert(0,0)
        ax.plot(np.array(D_t)/d_t,np.array(all_msd),label=name.split('_')[-1])
    ...
    ax.set_title("truncated")
    ax.set_xlabel(r'ion concentration')
...
def hist_rotation(out_path,figure_path):
    ...
    inpfile_str = "dbd_rotation_xpa_dna"
    str_contact = "contact_dbd_xpa_dna"
    outfile_str={}
    ...
    for name in outfile_str.keys():
        ...
        # array b1 slope: ---------this will be 2-d array when y is also 2-dimensional
        ...
        plt.savefig(figure_path + '\hist_rotation_'+name+'.png')   
            ...
            for bp_ind in file_pos:
                for pos_key in bp_ind:
                    for k in sorted(list(pos_key.keys())):
                        if k + d in pos_key.keys():
                            SD.append(((pos_key[k+d] - pos_key[k])*3.4)**2)       #1bp ~ 3.4A
                            #SD.append((pos_key[k+d] - pos_key[k])**2)
...
def hist_rotation_specific(out_path,figure_path):
    ...
    for name in outfile_str.keys():
        ...
        for f in outfile_str[name]:
            ...
            end_ind = []
            for num_ii in range(contact_num.__len__()):
                if contact_num[num_ii] > 0 and (not con_start) and modes[num_ii] == 0 :
                    con_start = True
                    start_ind.append(num_ii)
                if con_start and ( contact_num[num_ii] == 0 or modes[num_ii] != 0 ):
                    con_start = False
                    end_ind.append(num_ii-1)
...
def hist_mode_specific(out_path,figure_path):
    ...
    ax_s.set_ylim(0, 1.0)
    plt.figure(1)
    plt.savefig(figure_path + '\hist_mode_specific_s.png')
    ...
    ax_h.set_ylim(0, 0.2)
    plt.figure(2)
    plt.savefig(figure_path + '\hist_mode_specific_h.png')
    ...
    ax_d.set_ylim(0, 1.0)
    plt.figure(3)
    plt.savefig(figure_path + '\hist_mode_specific_d.png')
    #plt.show()
#<!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
#!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
#only certain snapshots is considered, where the contact between DNA and protein is formed at certain cumstance. .
...
def hist_rate_bound(out_path,figure_path):
    ...
    os.chdir(out_path)
    all_files = os.listdir('.')
    inpfile_str=["contact_dbd_xpa_dna"]
    outfile_str={}
    for s in inpfile_str:
        for f in all_files:
            if not( s in f):
...
cwd = os.getcwd()
read_path = '/scratch/wfli1/wwzhang/data/XPA/Full/outputfile_02'
out_path  = r'D:\temp\XPA\XPA_Full_out_02'
figure_path = r'D:\temp\XPA\full_figure_02'
